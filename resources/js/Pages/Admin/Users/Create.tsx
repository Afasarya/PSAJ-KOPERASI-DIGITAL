...
const CreateUser: React.FC<CreateUserProps> = ({ roles }) => {
  ...
  // Ensure role_id is not empty when submitting
  const validateAndSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!data.role_id) {
      // Set a default role_id if none is selected
      // This is just for form validation - the server will validate as well
      setData('role_id', roles[0]?.id.toString() || '1');
    }
    post(route('admin.users.store'));
  };
  ...
  return (
    <AdminLayout title="Tambah Pengguna Baru">
      ...
      <form onSubmit={validateAndSubmit} className="max-w-2xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle>Tambah Pengguna Baru</CardTitle>
            <CardDescription>
              Tambahkan pengguna baru ke sistem. Pastikan email belum pernah digunakan sebelumnya.
            </CardDescription>
          </CardHeader>
